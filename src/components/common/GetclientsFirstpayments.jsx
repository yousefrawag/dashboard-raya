...
const GetclientsFirstpayments = ({ data  , setparams}) => {
  // Process data to get status counts
  const projectStatus = data?.reduce((acc, item) => {
    const status = item?.firstPayment    ;
...
console.log("stauts" , projectStatus);
  ...
  return (
    <div className="flex items-center flex-col">
      <span className="text-lg font-bold mb-2">  أكثر قيمات الدفعة الأولى المتوفره مع العملاء</span>
      <Chart options={chartOptions} series={series} type="donut" width="380" />
    </div>
  );
};

export default GetclientsFirstpayments;
