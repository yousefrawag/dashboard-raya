...
const MostPorjectRequestsBycustomers = ({ data  , setparams}) => {
  // Process data to get status counts
  const projectStatus = data?.reduce((acc, item) => {
    const status = item?.project    ;
...
console.log("stauts" , projectStatus);
  ...
  return (
    <div className="flex items-center flex-col">
      <span className="text-lg font-bold mb-2">  أكثر  المشاريع إهتماما من قبل العملاء</span>
      <Chart options={chartOptions} series={series} type="donut" width="380" />
    </div>
  );
};

export default MostPorjectRequestsBycustomers;
