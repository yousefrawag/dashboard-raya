...
const AddExpenssModule = () => {
    const {                expensesmodule ,
                setExpensesmodule ,
          
                expenssEidt ,
                setExpensesEdit} = useDashboardContext()
    const [requerFiled , setRequirefiled] = useState("")
    const {data } = useQuerygetiteams("users" , "users")
    const { isError , isLoading , addIteam} = useQueryadditeam("expenss" , "expenss")
...
if(isLoading) {
  ...
  return (
    <div className={`fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center top-0 right-0 bottom-0  ${expensesmodule ? 
        "flex" :"hidden"
    }`}>
      <form onSubmit={handelsubmit} className="relative bg-white p-6 rounded-md shadow-lg w-full max-w-[500px] max-h-[400px] min-h-[200px]  mt-5 p-4 overflow-auto	">
        {/* Close Button */}
        <button
        onClick={() => setExpensesmodule(false)}
      ...
      <option value="">أختر</option>
{
  ...
  </div> <div className="mb-6 flex flex-col  gap-2">
    <label
      htmlFor="type"
      className="w-full text-lg font-medium text-black dark:text-white"
    >
...
إجمالى المبلغ
 ...
 <input type='number'  name='total'      className="focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary text-main p-3 w-full  outline-0 rounded-md border border-gray-300 shadow-sm focus:ring-blue-500"
 />
  </div>
...
export default AddExpenssModule;
