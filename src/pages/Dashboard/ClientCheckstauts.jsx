...
const ClientCheckstauts = () => {
    const {data , isLoading , isError} = useQuerygetiteams("clientcheckStauts", "clientcheckStauts")
    const {deleteIteam} = useQueryDelete("clientcheckStauts" , "clientcheckStauts")
...
const handelEdit = (item) => {
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName="أنواع المعاينات"/>
        {
          isAdmin || CanAdd ?         <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة معاينة جديد</button>
 : null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <MdouleAddCategoray  fetshkey="clientcheckStauts" titale="المعاينه"/>
 <EditmainCategory  fetshkey="clientcheckStauts" titale="المعاينة "/>
    </div>
  )
}

export default ClientCheckstauts