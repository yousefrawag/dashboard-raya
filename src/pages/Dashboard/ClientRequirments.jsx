...
const ClientRequirments = () => {
    const {data , isLoading , isError} = useQuerygetiteams("requirements", "requirements")
    const {deleteIteam} = useQueryDelete("requirements" , "requirements")
    const {module, setmodule , setmainCategory , setEditmaincategory} = useDashboardContext()
    console.log("regione" , data);
    
    const {CanAdd , CanDelte , CanEdit , CanView , isAdmin } = useGetUserAuthentications("requiremnts")
...
const handelEdit = (item) => {
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName="أنواع الطلبات"/>
        {
          isAdmin || CanAdd ?         <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة طلب جديد</button>
 : null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <AddLocationRegions  fetshkey="requirements" titale="الطلب"/>
 <EditLocationsRegion  fetshkey="requirements" titale="الطلب"/>

    </div>
  )
}

export default ClientRequirments