...
const CurrencyAPP = () => {
    const {data , isLoading , isError} = useQuerygetiteams("currency", "currency")
    const {deleteIteam} = useQueryDelete("currency" , "currency")
...
const handelEdit = (item) => {
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName="أنواع العملات"/>
        {
          isAdmin || CanAdd ?
        <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة عملة جديد</button>
: null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <MdouleAddCategoray  fetshkey="currency" titale="نوع العملة"/>
 <EditmainCategory  fetshkey="currency" titale="نوع العملة"/>
    </div>
  )
}
