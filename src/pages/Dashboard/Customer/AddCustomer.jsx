...
const AddCustomer = () => {
  ...
  const {data } = useQuerygetiteams("projects/selectproject" , "projects/selectproject")
    ...
    try {
     ...
     return    addIteam(data, {
          ...
          onError: (error) => {
            ...
            toast.error("يوجد خطأ في اضافة عميل - يرجي مراجعه البيانات مري اخري");
          },
        });
      }else{
        const data =  {
...
المشروع المهتم بة*
    ...
    <select name="project" required  onChange={handelInputschage}    value={formsData.project}           className="focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary text-main p-3 w-full  outline-0 rounded-md border border-gray-300 shadow-sm focus:ring-blue-500"
    >
            <option>قم بالإختيار</option>
...
تقسيط على كام  * {formsData.InstallmentType}
    ...
    <input
      ...
      name="installmentsPyYear" required 
      ...
      <option value="">أختر</option>
{
  ...
  <div className="mb-6 flex flex-col  gap-2 mt-5">
   ...
   {
    CurrenTap === "contacts" && <EmployeeCustomerContactnotes formsData={formsData} CustomerDealsatuts={formsData.CustomerDealsatuts} handelInputschage={handelInputschage} details={details} setDeatils={setDeatils} ContactDate={ContactDate} setContactDate={setContactDate} />
