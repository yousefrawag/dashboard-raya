...
const DropCustomers = () => {
    ...
    const handleFileChange = (e) => {
        ...
        reader.onload = (evt) => {
            ...
            setFileData(data);
            setStatus({ type: 'info', message: `تم قراءة ${data.length} عميل بنجاح. جاهز للرفع!` });
        };
    ...
    const handleDropCustomer = async () => {
        ...
        try {
            ...
            const res = await authFetch.post("/customers/drop-file", {
                customers: fileData
            });

            setStatus({ type: 'success', message: 'تم استيراد جميع العملاء بنجاح!' });
            setFileData([]);
            setFileName("");
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="max-full  mt-10 p-6 bg-white rounded-xl shadow-md">
            <div className="text-center mb-8">
                <h1 className="text-2xl font-bold text-gray-800">استيراد العملاء من إكسيل</h1>
                <p className="text-gray-500 mt-2">ارفع ملف الإكسيل لنقل بيانات عملائك إلى الـ CRM الجديد</p>
            </div>
