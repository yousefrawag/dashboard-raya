...
const UpdateDealyReport = () => {
    const {id} = useParams()
    const {data:currentDealiy , isLoading:loaddingget} = useQuerygetSpacficIteam("Dealiy-reports" , "Dealiy-reports" , id)
    const {data} = useQuerygetiteams("customers" , "customers")
const CurrentDealyItem =  currentDealiy?.data
    const [selectedUsers, setSelectedUsers] = useState([]); // Array for selected users
const [docs , setDocs] = useState([])
    const [search, setSearch] = useState(""); // Search input state
    const [users , setUsers] = useState()
    const [ReportType , setReportType] = useState("")
    const { updateiteam, isLoading } = useQueryupdate("Dealiy-reports", "Dealiy-reports");
...
useEffect(() => {
    ...
    if(CurrentDealyItem){
        setReportType(CurrentDealyItem?.ReportType)
        setSelectedUsers(CurrentDealyItem?.Customers)
    }
} , [data , CurrentDealyItem])
...
<option value="أخرى">أخرى</option>
                   ...
                   <div className="mb-6 flex flex-col gap-2">
                    ...
                    <textarea
                      defaultValue={CurrentDealyItem?.endcontact}
                        name="endcontact"
                        className="focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary text-main p-3 w-full outline-0 rounded-md border border-gray-300 shadow-sm focus:ring-blue-500"
                    ></textarea>
                </div> 
                ...
                <div className="mb-6 flex flex-col gap-2">
                    ...
                    <textarea
                        name="notes"
                        defaultValue={CurrentDealyItem?.notes}
                        className="focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary text-main p-3 w-full outline-0 rounded-md border border-gray-300 shadow-sm focus:ring-blue-500"
                    ></textarea>
                </div>
...
export default UpdateDealyReport;
