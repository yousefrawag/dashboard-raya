...
const FirstPayments = () => {
    const {data , isLoading , isError} = useQuerygetiteams("firstpayment", "firstpayment")
    const {deleteIteam} = useQueryDelete("firstpayment" , "firstpayment")
    const {module, setmodule , setmainCategory , setEditmaincategory} = useDashboardContext()
    console.log("regione" , data);
    
    const {CanAdd , CanDelte , CanEdit , CanView , isAdmin } = useGetUserAuthentications("appCurency")
  ...
  const columns = [
{
  ...
  cell: (row) => (
    <span>
      {Number(row.name).toLocaleString('en-US')}
    </span>
  )
}
,
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName="أنواع الدفعات"/>
        {
          isAdmin || CanAdd ?
        <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة دفعة جديد</button>
: null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <MdouleAddCategoray  fetshkey="firstpayment" titale="الدفعة الاولى"/>
 <EditmainCategory  fetshkey="firstpayment" titale="الدفعة الاولى"/>
    </div>
  )
}

export default FirstPayments