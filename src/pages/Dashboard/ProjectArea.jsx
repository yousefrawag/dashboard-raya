...
const ProjectArea = () => {
    const {data , isLoading , isError} = useQuerygetiteams("arae", "arae")
    const {deleteIteam} = useQueryDelete("arae" , "arae")
    const {module, setmodule , setmainCategory , setEditmaincategory} = useDashboardContext()

    
    const {CanAdd , CanDelte , CanEdit , CanView , isAdmin } = useGetUserAuthentications("arae")
...
const handelEdit = (item) => {
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName="مساحه العقار"/>
        {
          isAdmin || CanAdd ? 
        <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة حالة جديد</button>
: null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <MdouleAddCategoray  fetshkey="arae" titale="مساحه العقار"/>
 <EditmainCategory  fetshkey="arae" titale="مساحه العقار"/>
    </div>
  )
}
