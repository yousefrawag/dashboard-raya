...
const PropertyStauts = () => {
    const {data , isLoading , isError} = useQuerygetiteams("propertySatuts", "propertySatuts")
    const {deleteIteam} = useQueryDelete("propertySatuts" , "propertySatuts")
...
,
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName="حالات الشقق"/>
        {
          isAdmin || CanAdd ?
        <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة طابق جديد</button>
: null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <MdouleAddCategoray  fetshkey="propertySatuts" titale="الحالة"/>
 <EditmainCategory  fetshkey="propertySatuts" titale="الحالة"/>
    </div>
  )
}

export default PropertyStauts