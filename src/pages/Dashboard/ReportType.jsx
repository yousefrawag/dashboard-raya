...
const ReportType = () => {
    const {data , isLoading , isError} = useQuerygetiteams("ReportType", "ReportType")
    const {deleteIteam} = useQueryDelete("ReportType" , "ReportType")
    const {module, setmodule , setmainCategory , setEditmaincategory} = useDashboardContext()
    console.log("regione" , data);
    
    const {CanAdd , CanDelte , CanEdit , CanView , isAdmin } = useGetUserAuthentications("CustomerTypes")
...
const handelEdit = (item) => {
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName=" نوع التقرير"/>
        {
          isAdmin || CanAdd ? 
        <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة نوع جديد</button>
: null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <AddLocationRegions  fetshkey="ReportType" titale=" نوع التقرير "/>
 <EditLocationsRegion  fetshkey="ReportType" titale=" نوع التقرير "/>
    </div>
  )
}
