...
const AddImportExportOrdears = () => {
  ...
  // ✅ endpoint إضافة الطلب
  const { isLoading: addingOrder, addIteam } = useQueryadditeam(
    'ordears',
    'ordears'
  );
      ...
      addIteam(cleanedData, {
        ...
        onError: (error) => {
          if (error?.response?.status === 404) {
            return toast.error('هذا الطلب موجود من قبل');
          }
          toast.error(
            'يوجد خطأ في إضافة الطلب - يرجى مراجعة البيانات مرة أخرى'
          );
        },
      });
    } catch (error) {
      toast.error('حدث خطأ أثناء إضافة الطلب - يرجى المحاولة مرة أخرى');
    }
  };
      ...
      >
            ...
            <div className="flex flex-col gap-2 lg:col-span-2 relative">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  // لو المستخدم بيكتب من جديد، نصفّر الـ clientId
                  if (formData.clientId) {
        ...
        <div className="add_return flex justify-between items-center mt-4 shadow-lg p-4 bg-white dark:bg-form-input rounded-md">
          <div className="add_btn">
            ...
            >
              <FiPlus />
              {addingOrder ? 'جاري الإضافة...' : 'إضافة الطلب'}
            </button>
          </div>
...
export default AddImportExportOrdears;