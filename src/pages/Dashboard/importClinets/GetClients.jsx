  ...
  //     },
      {
        ...
        cell: (row) => (
                  <Link   target="_blank" 
  rel="noopener noreferrer" to={`/import-clinets/${row._id}`} >{row.fullName}</Link>
        )
      },
  ...
  cell: (row) => {
    ...
    title={lastFollow.lastContact}
      >
        ...
        {
          handelStringLength( lastFollow.lastContact)
        }
      {
...
if(isLoading){
  ...
  return (
    <div>
        <HeadPagestyle isAdmin={isAdmin} CanAdd={CanAdd}  pageName="العملاء" to="/import-clinets/add" title="إضافة عميل"/>
