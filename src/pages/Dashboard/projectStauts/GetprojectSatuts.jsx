...
const GetprojectSatuts = () => {
    const {data , isLoading , isError} = useQuerygetiteams("projectStatuts", "projectStatuts")
    const {deleteIteam} = useQueryDelete("projectStatuts" , "projectStatuts")
    const {module, setmodule , setmainCategory , setEditmaincategory} = useDashboardContext()
    console.log("regione" , data);
    
    const {CanAdd , CanDelte , CanEdit , CanView , isAdmin } = useGetUserAuthentications("projectstuts")
...
const handelEdit = (item) => {
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName="حالات العقارات"/>
        {
          isAdmin || CanAdd ?         <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة حالة جديد</button>
 : null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <MdouleAddCategoray  fetshkey="projectStatuts" titale="حالة العقار"/>
 <EditmainCategory  fetshkey="projectStatuts" titale="حالة العقار" />
    </div>
  )
}

export default GetprojectSatuts