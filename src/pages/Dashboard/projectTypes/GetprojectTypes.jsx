...
const GetprojectTypes = () => {
    const {data , isLoading , isError} = useQuerygetiteams("region", "region")
...
const handelEdit = (item) => {
  ...
  return (
    <div>
        ...
        <Breadcrumb  pageName="أنواع العقارات"/>
        {
          isAdmin || CanAdd ?         <button onClick={( ) => setmodule(true)} className="block text-white bg-main hover:bg-main2 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center  dark:focus:ring-blue-800"> إضافة نوع جديد</button>
: null
        }
        </div>
...
<CustomeTabel  data={data?.data?.data} columns={columns}/>
 ...
 <AddLocationRegions  fetshkey="region" titale="نوع العقار"/>
 <EditLocationsRegion  fetshkey="region" titale="نوع العقار"/>
    </div>
  )
}
