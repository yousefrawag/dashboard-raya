...
const ProjectArchiev = () => {
  const { data, isLoading } = useQuerygetiteams("projects/archiev", "projects");
...
try {
...

   {
      ...
      cell: (row) => (
        <div className="flex items-center justify-center space-x-3.5 gap-5">
          ...
          {
            isAdmin || CanDelte ?
              <button className="hover:text-red-500" onClick={() => SendeProjectToarchev(row._id  , "process")}>
                <RxReload size={20} />
              </button> : null
          }
                 {
  ...
  return (
    <div>
      <HeadPagestyle pageName=" أرشيف المشاريع" isAdmin={isAdmin} CanAdd={CanAdd} to="/Add-project" title="إضافة مشروع" />
