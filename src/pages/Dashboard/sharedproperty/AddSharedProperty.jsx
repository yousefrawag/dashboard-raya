...
const AddSharedProperty = () => {
  const { isLoading, addIteam } = useQueryadditeam("sharedProperty", "sharedProperty");
...
const formDataWithfile = new FormData();
    ...
    try {
      ...
      const payload = {
        ...
        project: {
          ...formData.project,
          // Combine all files (images + docs) into imagesURLs
          imagesURLs: [...images_video, ...docs],
        },
      };
       const files = payload.project.imagesURLs || [];
formDataWithfile.append("data" , JSON.stringify(payload))
  if (files.length > 0 && files[0] instanceof File) {
      files.forEach((file) => formDataWithfile.append('files', file));
    }
      // Send to API via useQueryadditeam
       addIteam(formDataWithfile , {
        onSuccess:()=>{
            toast.success("تم اضافه طلب مشروع جديد")
            return navigate("/sharedProperty")
        }
       });
  ...
  return (
    ...
    >
      ...
      <div className="dark:bg-form-input flex items-center shadow-lg gap-4 mb-4 w-full h-full p-4 bg-white rounded-[10px]">
        ...
        <p className="font-semibold text-lg">إضافة طلب عقار جديد</p>
      </div>
...
export default AddSharedProperty;