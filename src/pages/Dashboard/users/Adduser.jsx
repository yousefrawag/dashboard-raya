...
const Adduser = () => {
  ...
  const [userData, setUserData] = useState({
    ...
    role: "",
    type: "",
    institution: "",          // تخزين _id المؤسسة
    allowedProjects: [],      // مصفوفة من _id المشاريع
  });
...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!userData.password) return toast.error("يجب إضافة كلمة المرور");
    if (userData.type === "InstitutionsUser" && !userData.institution) {
      return toast.error("يجب اختيار المؤسسة التابع لها");
    }

    // تحضير البيانات للإرسال (باستخدام JSON)
    const payload = {
      ...userData,
      // تحويل allowedProjects إلى مصفوفة من المعرفات (هي كذلك بالفعل)
    };
    ...
    try {
      await addIteam(formData, {
        ...
        onError: (error) => {
          const msg = error.response?.data?.mesg || "حدث خطأ أثناء الإضافة";
          toast.error(msg);
        }
      });
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className='w-full h-full bg-white rounded-[10px] dark:bg-form-input'>
      ...
      <div className='main-section w-full max-h-[400px] min-h-[100px] p-4 overflow-auto'>
        ...
        {/* حقول تظهر فقط عند اختيار "تابع لمؤسسة" */}
